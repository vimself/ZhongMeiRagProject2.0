...
| `/api/v2/knowledge-bases` | POST | 创建知识库，自动 owner | 登录 |
| `/api/v2/knowledge-bases/{id}` | GET | 获取详情 | viewer |
| `/api/v2/knowledge-bases/{id}` | PUT | 更新名称/描述 | editor |
| `/api/v2/knowledge-bases/{id}` | DELETE | 停用（软删除） | owner |
| `/api/v2/knowledge-bases/{id}/permissions` | GET | 查看权限列表 | viewer |
| `/api/v2/knowledge-bases/{id}/permissions` | PUT | 更新权限矩阵 | owner |

**管理员路由（`/api/v2/admin/knowledge-bases`）**

| 端点 | 方法 | 功能 |
|------|------|------|
| `/api/v2/admin/knowledge-bases` | GET | 查看全部知识库，支持搜索和状态筛选 |
...
- `KnowledgeBaseOut`：完整输出，含 `my_role` 字段
...
- `backend/app/api/knowledge_base_deps.py`：权限检查依赖
  ...
  - admin 用户自动通过所有权限检查
- `GET /api/v2/knowledge-bases/{id}/permission-candidates`：owner/admin 可查询可授权用户候选列表，供权限抽屉选择用户。
- `GET /api/v2/admin/knowledge-bases/{id}/permissions`：管理员可查看任意知识库（包括已停用知识库）的权限记录。
...
| 测试类 | 覆盖范围 |
|--------|---------|
| `TestKnowledgeBaseCRUD` (9) | 创建自动 owner、审计日志、列表、搜索、获取详情、更新、停用、404 |
...
| `TestAdminKnowledgeBase` (5) | 管理员列表全部 KB、搜索、状态筛选、查看停用 KB 权限、普通用户 403 |
...
- `frontend/src/api/knowledge.ts`：`listKnowledgeBases`、`createKnowledgeBase`、`getKnowledgeBase`、`updateKnowledgeBase`、`disableKnowledgeBase`、`listPermissions`、`listPermissionCandidates`、`updatePermissions`、`adminListKnowledgeBases`、`adminListPermissions`。
...
- 工具栏：搜索输入框、刷新、新建知识库按钮。
- 新建/编辑对话框：名称（必填）、描述（选填）。
- 权限管理抽屉：查看/添加/修改/删除权限，角色选择（owner/editor/viewer）。
- 操作按钮根据 `my_role` 动态显示：editor 可编辑，owner 可编辑+权限+停用。
- 空状态展示。

**AdminKnowledgeBasesView（管理员知识库管理）**

- 表格：知识库名称、描述、状态、创建者、创建时间、操作。
- 工具栏：搜索输入框、状态筛选、刷新。
- 权限查看抽屉（只读）。
- 空状态展示。
...
- 修复知识库更新、停用、权限变更审计日志 `actor_user_id` 为空的问题，现在统一记录当前操作者。
...
- 新增 `GET /api/v2/admin/knowledge-bases/{id}/permissions`，管理员在查看已停用知识库时仍可查看权限记录。
...
- 补充后端测试覆盖：审计操作者、缺失用户、重复用户、授权候选用户、停用知识库权限查看。
...
- ✅ 知识库 CRUD（创建、列表、详情、更新、停用）
- ✅ 权限矩阵 owner/editor/viewer 三级权限
- ✅ owner/admin 可查询授权候选用户
- ✅ admin 用户完全管理权限
- ✅ admin 可查看已停用知识库的权限记录
- ✅ 创建知识库自动 owner
- ✅ 禁止移除最后一个 owner
- ✅ 非法角色 Pydantic 校验
- ✅ 不存在用户、重复用户权限更新被明确拒绝
