...
| `/api/v2/knowledge-bases/{id}` | DELETE | 删除（软删除） | admin |
...
| `/api/v2/admin/knowledge-bases/{id}/permissions` | GET | 查看任意知识库权限记录（包括已删除知识库快照） |
...
- `frontend/src/api/knowledge.ts`：`listKnowledgeBases`、`createKnowledgeBase`、`getKnowledgeBase`、`updateKnowledgeBase`、`disableKnowledgeBase`、`listPermissions`、`listPermissionCandidates`、`updatePermissions`、`adminListKnowledgeBases`、`adminListPermissions`、`adminListPermissionCandidates`、`adminUpdatePermissions`。
...
- 工具栏：搜索输入框、刷新。
- 编辑对话框：名称（必填）、描述（选填）。
- 操作按钮根据 `my_role` 动态显示：所有可访问知识库均可进入文档，owner/editor/admin 可编辑基础信息；不再提供新建、删除和权限设置。
- 空状态展示。

**AdminKnowledgeBasesView（管理员知识库管理）**

- 表格：知识库名称、描述、状态、文档规模、成员数、创建者、创建时间、删除时间、操作。
- 工具栏：搜索输入框、状态筛选、刷新、新建知识库。
