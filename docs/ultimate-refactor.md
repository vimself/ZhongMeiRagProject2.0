...
以下内容是对“设计目标”和“当前真实实现”的校准说明：

- 当前代码已接入 `qwen3-rerank`，并在 `RRF` 之后对候选 chunk 做官方重排序。
- 当前聊天链路已补上最近会话历史感知的 query rewrite，不再是严格单轮检索。
- 当前最大的准确度缺口已经不是“有没有 rerank”，而是 **chunk/page/bbox 锚点仍不够真实**，以及 `knowledge_page_index_v2` 尚未进入检索闭环。
- 上传到问答全链路的专项审查见 [rag-accuracy-audit-2026-05-13.md](rag-accuracy-audit-2026-05-13.md)。
