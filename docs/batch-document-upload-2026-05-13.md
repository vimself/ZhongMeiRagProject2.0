...
- 上传接口请求超时放宽到 1 小时，避免大批量 PDF 在慢网络下误判失败。
- 上传完成后展示成功入队数量，并刷新文档列表继续沿用原有进度轮询。
- 若网关返回 `413`，前端明确提示文件总大小超过服务端上传上限。
...
- 单次最多上传 50 份 PDF；单文件大小仍受 `UPLOAD_MAX_MB` 控制，默认 200MB。Nginx `client_max_body_size` 需覆盖批量请求总大小，默认配置为 `10g`，用于匹配 50 份 200MB PDF 的批量请求上限。
