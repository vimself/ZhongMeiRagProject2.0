...
- **OCR 解析**：GLM-OCR 官方 self-hosted pipeline 负责 PDF 页面渲染、PP-DocLayoutV3 版面检测、区域识别、Markdown 分页输出、图片裁剪和 JSON 版面信息；`GlmOcrApi` 在官方结果后追加文本质量修复，针对扫描水印 PDF 的长输出重复、百分比数字失控、公式标记异常、目录章节号粘连和多行编号条文，按深色文字投影切行为短图逐行重识别；针对带可抽取文本层 PDF，在同一 GLM 版面 bbox 内保守使用文本层纠正章节号漂移、重复小节号和长段语义扩写；落盘前对 `1111111 电气`、`111.1`、`11.1.1.1 2`、`11.111.18.2`、`12.12.` 等章节号畸形做最终规范化，同时避免误改 `1110V`、`1110kV` 等工程数值；并对 `97%～9999%`、破损 `\mathrm{左}`、`\sqrt{}`、`\sum G` 等高确定性工程符号噪声做窄规则规范化，输出 `quality_report.json` 并在 `metadata.json` 记录修复次数；`GlmOcrApi/ocrtest/run_full_pdf_quality_audit.py` 是全 PDF 分批质量门禁，明显畸形章节号会作为 blocker；当前 `泵站设计标准.pdf` 117 页评测基线为质量分 98.77、阻断页 0、修复区域 673，章节 11 专项回归 `pump-section11-codex-v3` 第 76-78 页质量分 100、`pump-section11-late-codex-v5` 第 97-102 页质量分 99.67 且阻断页 0，`DB32T 4077.1-2021 矿山生态修复工程技术规程1+通则.pdf` 98 页评测基线为质量分 99.64、阻断页 0、修复区域 214；后端 `GlmOCRClient` 保持上传-轮询-下载协议，OCR/vLLM 短时不可用按临时异常退避重试；`GlmOcrApi/start.sh` 默认过滤 PyTorch/NCCL TCPStore `Broken pipe` 心跳噪声，保留 vLLM/API warning/error 和 OCR 任务级关键日志；日志过滤通过持久 `nohup bash` 管道 wrapper 启动，`stop.sh` 按进程组停止，禁止改回会在父脚本退出后关闭过滤器的 process substitution。
