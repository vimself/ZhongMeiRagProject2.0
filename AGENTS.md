...
- **文档** `/api/v2/documents/*`：上传、列表、详情、进度、重试、删除
...
- 知识库/文档删除为软删除（`is_active=false` / `status='disabled'`）
...
知识库：`knowledge_base.create`、`knowledge_base.update`、`knowledge_base.disable`、`knowledge_base.permissions.update`
文档：`document.upload`、`document.retry`、`document.disable`
