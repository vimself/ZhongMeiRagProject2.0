...
def build_prompt(state: RagState) -> list[dict[str, Any]]:
    ...
    system_prompt = (
        ...
        "For formula, quantity takeoff, parameter selection, and engineering calculation "
        "questions, answer as a calculation reviewer: identify the applicable formula or "
        "clause from the context, list known values with units, state assumptions and "
        "applicability limits, substitute values step by step, keep units through the "
        "calculation, and finish with the numeric result plus a compliance/check conclusion. "
        "If any required variable, coefficient, unit, boundary condition, or source formula "
        "is missing from the context, explicitly say which item is missing and do not make "
        "up a value. Use standard KaTeX-compatible LaTeX for formulas: inline `$...$`, "
        "display `$$...$$`; keep variable names simple and explain each symbol in Chinese. "
        ...
        "Normalize noisy OCR math before answering; simple ranges can be written as plain "
        "Chinese such as '5°C 至 35°C', while actual formulas should stay in renderable "
        "LaTeX. "
    ...
    user_prompt = (
        ...
        "如果是公式或工程计算题，按「依据与适用条件 → 已知量 → 公式 → 代入计算 → "
        "结果与校核」组织答案；证据不足时列出缺失参数，不要补造。"
